...
// selectWorkspace gets a list of available workspaces and then checks
...
func (m *Meta) selectWorkspace(b backend.Backend) error {
	...
	if err != nil {
		return fmt.Errorf("Failed to get available workspaces: %s", err)
	}
	if len(workspaces) == 0 {
		return fmt.Errorf(errBackendNoAvailableWorkspaces)
	}
	...
	v, err := m.UIInput().Input(context.Background(), &terraform.InputOpts{
		Id: "select-workspace",
		Query: fmt.Sprintf(
			"[reset][bold][yellow]The currently selected workspace (%s) is not available.[reset]",
			workspace),
...
const errBackendNoAvailableWorkspaces = `
No available workspaces. Use the "terraform workspace" command to create
